...
function registraItem(req,res){
	...
	var item = new Chancadordata(req.body);
	   ...
	   .exec(
	   		(err, itemsFound) => {
	   			if (err){
	   				...
	   				//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	   				Chancadordata.countDocuments({}, (err,conteo) =>{
	   					res.status(200).send({
								items: itemsFound,
								total: conteo
						});
...
function itemsRangoFechas(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION Y CAMPOS SEGÚN LA CONSULTA
	Chancadordata.find({'idn': idnode,tm : {
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Chancadordata.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
