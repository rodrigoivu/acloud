...
function registraItem(req,res){
	...
	var item = new Harnerodata(req.body);
	   ...
	   .exec(
	   		(err, itemsFound) => {
	   			if (err){
	   				...
	   				//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	   				Harnerodata.countDocuments({}, (err,conteo) =>{
	   					res.status(200).send({
								items: itemsFound,
								total: conteo
						});
...
function itemsRangoFechas(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION Y CAMPOS SEGÚN LA CONSULTA
	Harnerodata.find({'idn': idnode, tm : {
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Harnerodata.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
