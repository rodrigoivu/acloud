...
var Configpilas = require('../models/configpilas');
var idItem;
	...
	item.save((err, itemStored) => {
		if(err){
			...
			if(!itemStored){
				...
				console.log("idItemConfigpilas: "+idItem);
			}
		}
	});
}
	   ...
	   .exec((err, itemsFound) => {
   			if (err){
   				...
   				if(!itemsFound){
					...
					console.log("si hay datos de configpilas id:"+idItem);
				}
   			}
   		});
}
...
function actualizaItem(req,res){
	...
	var params = req.body;      
 	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Configpilas.findByIdAndUpdate(itemId, params, { new: true }, (err, itemUpdated) => { 
		if(err){
			res.status(500).send({
				error: err,
...
function uploadImage(req,res){
	...
	if(req.files){
		...
		if( extensionesValidas.indexOf(file_ext) >= 0 ){
			//personalizar Nombre
			var nombreArchivo = `pilas.jpg`;
			var path_destino = `./uploads/imgpilas/${nombreArchivo}`;
			//Mover archivo
			fs.rename( file_path,path_destino, function(err){
				if (err){
					...
					Configpilas.findByIdAndUpdate(configId, {image: nombreArchivo}, (err, itemUpdated) => {
						if(!itemUpdated){
...
function getImageFile(req,res){
	...
	var path_file = path.resolve(__dirname, `../uploads/imgpilas/${ imageFile }`);
