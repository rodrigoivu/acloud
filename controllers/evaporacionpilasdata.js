...
function registraItem(req,res){
	...
	var item = new Evaporacionpilasdata(req.body);
	   ...
	   .exec(
	   		(err, itemsFound) => {
	   			if (err){
	   				...
	   				//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	   				Evaporacionpilasdata.countDocuments({}, (err,conteo) =>{
	   					res.status(200).send({
								items: itemsFound,
								total: conteo
						});
...
function itemsRangoUltimos(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION Y CAMPOS SEGÚN LA CONSULTA
	Evaporacionpilasdata.find({'idn': idnode})
	   .skip(0)
	   .limit(items)
	   .sort([['tm', -1]])
	   //.sort({ _id: 'desc' })
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Evaporacionpilasdata.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
